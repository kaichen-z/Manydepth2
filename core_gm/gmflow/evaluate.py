...
def validate_kitti(model,
    ...
    for val_id in range(len(val_dataset)):
        ...
        print(image1[0, :, 0, 0])
        print(image2[0, :, 0, 0])
        #print(attn_splits_list, corr_radius_list, prop_radius_list)
        results_dict = model(image1.repeat(8,1,1,1), image2.repeat(8,1,1,1),
                             attn_splits_list=attn_splits_list,
                             corr_radius_list=corr_radius_list,
                             prop_radius_list=prop_radius_list,
                             )
        ...
        # useful when using parallel branches
        flow_pr = results_dict['flow_preds'][-1]
        print('flow----', flow_pr[0].permute(1,2,0).sum()) #8848452.
        bgr_for_vis0 = flow_vis.flow_to_color((flow_pr[0].permute(1,2,0).cpu().numpy()), convert_to_bgr=True)
        img1m_cv = image1[0].cpu().permute(1,2,0).numpy()
        img2m_cv = image2[0].cpu().permute(1,2,0).numpy()
        cv2.imwrite('test_eval/%s_FLOW_1.png'%val_id, np.concatenate((bgr_for_vis0, img1m_cv, img2m_cv), 1))
        flow = padder.unpad(flow_pr[0]).cpu()
