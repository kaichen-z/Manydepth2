...
class Multi_Head_Attention_Module(nn.Module):
    def __init__(self, in_channel, out_channel, k_size=[3, 5, 7]):
        super(Multi_Head_Attention_Module, self).__init__()
        self.ca0 = EChannelAttention(k_size=k_size[0])
        self.ca1 = EChannelAttention(k_size=k_size[1])
        self.ca2 = EChannelAttention(k_size=k_size[2])
        self.conv_se = nn.Conv2d(in_channels = in_channel, out_channels = out_channel, kernel_size = 3, stride = 1, padding = 1 )
        self.relu = nn.ReLU(inplace = True)
    def forward(self, features):
        features = (self.ca0(features) + self.ca1(features) + self.ca2(features))/3
        return self.relu(self.conv_se(features))
...
class Multi_Head_Attention(nn.Module):
    def __init__(self, k_size=[3, 5, 7]):
        ...
        self.avg_pool = nn.AdaptiveAvgPool2d(1)
        self.conv_3 = nn.Conv1d(1, 1, kernel_size=k_size[0], padding=(k_size[0] - 1) // 2, bias=False) 
        self.conv_5 = nn.Conv1d(1, 1, kernel_size=k_size[1], padding=(k_size[1] - 1) // 2, bias=False) 
        self.conv_7 = nn.Conv1d(1, 1, kernel_size=k_size[2], padding=(k_size[2] - 1) // 2, bias=False) 
        self.sigmoid = nn.Sigmoid()

    def forward(self, x):
        y = self.avg_pool(x)
        y3 = self.conv_3(y.squeeze(-1).transpose(-1, -2)).transpose(-1, -2).unsqueeze(-1)
        y5 = self.conv_5(y.squeeze(-1).transpose(-1, -2)).transpose(-1, -2).unsqueeze(-1)
        y7 = self.conv_7(y.squeeze(-1).transpose(-1, -2)).transpose(-1, -2).unsqueeze(-1)
        y = (y3 + y5 + y7) / 3
        y = self.sigmoid(y)
        return x * y.expand_as(x)
