...
class CityscapesEvalDataset(MonoDataset):
    ...
    def get_colors(self, city, frame_name, side, do_flip):
        ...
        future_name = self.get_offset_framename(frame_name, offset=+2)
        future_color = self.get_color(city, future_name, side, do_flip, is_sequence=True)

        inputs = {}
        inputs[("color", 0, -1)] = color
        inputs[("color", -1, -1)] = prev_color
        inputs[("color", 1, -1)] = future_color
